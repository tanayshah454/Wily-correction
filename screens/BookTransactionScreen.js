...
export default class TransactionScreen extends React.Component {
    ...
    checkStudentEligibilityForBookIssue=async()=>{
      ...
      if(studentref.docs.length===0){
        ...
        alert("student is not registered in the database")
      }else{
        studentref.docs.map((docs)=>{
          ...
          if(student.numberOfBooksIssued<2){
            ...
            isStudentEligible=false
            alert('student has already issued maximum amount of books')
          }
        })
      }
    ...
    checkStudentEligibilityForBookReturn=async()=>{
      const transactionref=db.collection('transactions').where('bookId','==',this.state.bookId).limit(1).get()
      var isStudentEligible=''
      transactionref.docs.map((doc)=>{
        ...
        if(lastBookTransaction.studentId===this.state.studentId){
          ...
          isStudentEligible=false
          alert('Student has not issued this book')
        }
      })
      return(isStudentEligible)
    }
